...
Require Import MyNatBinomialHeap.

Import ListNotations.

Lemma benchmark_main_result :
  benchmark_main =
    let my10 := make_my10 MyO in
    MyS (my_mul (my_add my10 my10) (my_mul my10 my10)).
Proof. vm_compute. reflexivity. Qed.

(** [MyO] is zero and every [MyS] is one heap cell.  In particular, this
    deliberately does not use the built-in [nat] instance, whose encoding is
    a machine number. *)
Fixpoint encode_mynat (n : MyNat) : FunValues.Value :=
  match n with
  | MyO => FunValues.Num 0
  | MyS n' => value_list_of_values [encode_mynat n']
  end.

Global Instance Refinable_MyNat : Refinable MyNat :=
  {| encode := encode_mynat |}.

Lemma auto_mynat_MyO : forall env s,
  env |-- ([FunSyntax.Const 0], s) ---> ([encode MyO], s).
Proof.
  intros.
  constructor.
Qed.

Ltac2 auto_mynat_MyO_tac (_r : unit -> unit) :=
  match! goal with
  | [ |- ?env |-- (_, _) ---> ([?encoded], _) ] =>
      let (_, args) := Constr.decompose_app encoded in
      let result := Array.get args (Int.sub (Array.length args) 1) in
      lazy_match! result with
      | MyO => refine open_constr:(auto_mynat_MyO $env _)
      end
  end.

Lemma auto_mynat_MyS : forall env s x n,
  env |-- ([x], s) ---> ([encode n], s) ->
  env |-- ([Op Cons [x; FunSyntax.Const 0]], s)
    ---> ([encode (MyS n)], s).
Proof.
  intros.
  ltac1:(Eval_eq).
...
Ltac2 auto_mynat_MyS_tac (recur : unit -> unit) :=
  match! goal with
  | [ |- ?env |-- (_, _) ---> ([?encoded], _) ] =>
      let (_, args) := Constr.decompose_app encoded in
      let result := Array.get args (Int.sub (Array.length args) 1) in
      lazy_match! result with
      | MyS ?n =>
          refine open_constr:(auto_mynat_MyS $env _ _ $n
...
Ltac2 Set relCompilerDB as olddb :=
  fun recur =>
    Control.plus (fun () => olddb recur)
      (fun _ => auto_mynat_MyO_tac recur).

Ltac2 Set relCompilerDB as olddb :=
  fun recur =>
    Control.plus (fun () => olddb recur)
      (fun _ => auto_mynat_MyS_tac recur).

Lemma auto_mynat_case : forall {A} `{Refinable A}
    env s name xn n xzero xsucc (fzero : A) (fsucc : MyNat -> A),
  env |-- ([xn], s) ---> ([encode n], s) ->
  match n with
  | MyO => env |-- ([xzero], s) ---> ([encode fzero], s)
  | MyS n' =>
      FEnv.insert (name_enc name, Some (encode n')) env
        |-- ([xsucc], s) ---> ([encode (fsucc n')], s)
  end ->
  env |--
    ([FunSyntax.If FunSyntax.Equal [xn; FunSyntax.Const 0]
       xzero
       (Let (name_enc name) (Op Head [xn]) xsucc)], s)
    ---> ([encode (match n with
                   | MyO => fzero
                   | MyS n' => fsucc n'
                   end)], s).
...
Ltac2 auto_mynat_case_tac (recur : unit -> unit) :=
  match! goal with
  | [ |- ?env |-- (_, _) ---> ([?encoded], _) ] =>
      let (_, args) := Constr.decompose_app encoded in
      let result := Array.get args (Int.sub (Array.length args) 1) in
      ...
      | (match ?n with
         ...
         | MyS n' => @?fsucc n'
          ...
          let names_in_cenv := List.map
            ...
            (Control.hyps ()) in
          let branch_names :=
            binders_names_of_constr_lambda fsucc names_in_cenv in
          let name := List.nth branch_names 0 in
          refine open_constr:(auto_mynat_case
            $env _ $name _ $n _ _ $fzero $fsucc
            ltac2:(Control.enter recur)
            ltac2:(destruct $n at 1; Control.enter (fun () =>
              rewrite_lowerable ();
...
Ltac2 Set relCompilerDB as olddb :=
  fun recur =>
    Control.plus (fun () => olddb recur)
      (fun _ => auto_mynat_case_tac recur).
...
Theorem my_ltb_equation : ltac2:(unfold_fix_type '@my_ltb).
Proof. unfold_fix_proof '@my_ltb. Qed.
...
Theorem my_add_equation : ltac2:(unfold_fix_type '@my_add).
Proof. unfold_fix_proof '@my_add. Qed.

Theorem my_mul_equation : ltac2:(unfold_fix_type '@my_mul).
Proof. unfold_fix_proof '@my_mul. Qed.

Derive my_ltb_prog
  in ltac2:(relcompile_tpe 'my_ltb_prog 'my_ltb [])
  as my_ltb_prog_correct.
Proof. relcompile. Qed.

Derive smash_prog
  in ltac2:(relcompile_tpe 'smash_prog 'smash ['my_ltb])
...
Derive find_max'_prog
  in ltac2:(relcompile_tpe 'find_max'_prog 'find_max' ['my_ltb])
...
Derive delete_max_aux_prog
  in ltac2:(relcompile_tpe 'delete_max_aux_prog 'delete_max_aux
    ['heap_delete_max; 'my_ltb])
...
Derive my_add_prog
  in ltac2:(relcompile_tpe 'my_add_prog 'my_add [])
  as my_add_prog_correct.
Proof. relcompile. Qed.

Derive my_mul_prog
  in ltac2:(relcompile_tpe 'my_mul_prog 'my_mul ['my_add])
  as my_mul_prog_correct.
Proof. relcompile. Qed.

Derive make_my10_prog
  in ltac2:(relcompile_tpe 'make_my10_prog 'make_my10 [])
  as make_my10_prog_correct.
Proof. relcompile. Qed.

Derive benchmark_main_prog
  in ltac2:(relcompile_tpe 'benchmark_main_prog 'benchmark_main
    ['make_my10; 'my_add; 'my_mul; 'make_list; 'insert_list; 'merge;
      'delete_max])
