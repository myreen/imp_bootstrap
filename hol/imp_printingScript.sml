...
(* S-expression type for pretty printing IMP programs *)
...
(* Parameter name list to s-expression *)
...
(* Function and program to s-expression *)
...
Definition funcs2s_def:
  funcs2s []      = SEmpty ∧
  funcs2s (f::fs) = SPair (func2s f) (funcs2s fs)
End

Definition prog2s_def:
  prog2s (Program funcs) = funcs2s funcs
End

(* Convert s-expression to string *)

Definition sexpr2str_def:
  sexpr2str (SNum n)      = "'" ++ num2str n ∧
  sexpr2str (SStr s)      = s ∧
  sexpr2str (SPair s1 s2) = sexpr2str s1 ++ " " ++ sexpr2str s2 ∧
  sexpr2str SEmpty        = "" ∧
  sexpr2str (SBlock s)    = "(" ++ sexpr2str s ++ ")"
End

(* Top-level: IMP program to string *)

Definition imp2str_def:
  imp2str p = sexpr2str (prog2s p)
End
