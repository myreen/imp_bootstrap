(** Original source copied from:
    ...
    License terms: [../../benchmarks/binomial_heap/NOTICE].
...
Fixpoint delete_max_aux (m: key) (p: priqueue) : priqueue * priqueue :=
  match p with
  | Leaf :: p'   => let/d tmp := delete_max_aux m p' in let (j,k) := tmp in (Leaf::j, k)
  | Node x t1 Leaf :: p' =>
       if m >? x
       then (let/d tmp := delete_max_aux m p' in let (j,k) := tmp
...
Definition delete_max (q: priqueue) : option (key * priqueue) :=
  ...
  | None => None
  | Some  m => let/d tmp := delete_max_aux m q in let (p',q') := tmp
